...
import {runText, type CharTerm, type RGB} from '@bun-win32/terminal'
import type {AudioSource, PlaybackMode} from './types'
...
async function playVideo(path: string): Promise<void> {
	...
	let mode: PlaybackMode = process.env.VIDEO_MODE === 'ascii' ? 'ascii' : 'half'
	let paused = false
	let turbo = false
	let displayedTs = 0
	let everDrew = false
